/* tmux_textinput_wrap.c - Mini-app for tmux-driven soft-wrap e2e tests.
 *
 * Mounts a soft-wrapping multiline TuiTextInput at row 1 with a "> " prompt,
 * runs the standard bubbletea-style runtime, and tracks SIGWINCH by updating
 * the textinput's terminal_width on WINDOW_SIZE messages. Soft-wrap is on, so
 * long logical lines wrap onto continuation rows instead of scrolling
 * horizontally. No quit key — the driving test tears down with
 * `tmux kill-session`.
 */
